...
void rightWheelWrite() {
  if(packetBuffer[RIGHT] < 0x7f) { // original was 0xbb
    Serial.println("BB");
    motorWrite(RED_LED, packetBuffer[RIGHT]); //If byte 2 was 0xbb then this writes the speed from byte 3 to the pin for RED_LED
    analogWrite(31, 255);
  }
  
  if(packetBuffer[RIGHT] >= 0x7f) { // original was 0xaa
    Serial.println("AA");
    motorWrite(31, packetBuffer[RIGHT]); //If byte 2 was 0xaa then this writes the speed from byte 3 to pin 31
    analogWrite(RED_LED, 255);
  }
}

void leftWheelWrite() {
  if(packetBuffer[LEFT] < 0x7f) { //This is checking the hex value of byte 0 for the direction
    motorWrite(GREEN_LED, packetBuffer[LEFT]); //If byte 0 was 0xbb then this writes the speed from byte 1 to the pin for GREEN_LED
    analogWrite(YELLOW_LED, 255);             
  }
  
  if(packetBuffer[LEFT] >= 0x7f) {
    motorWrite(YELLOW_LED, packetBuffer[LEFT]); //If byte 0 was 0xaa then this writes the speed from btye 1 to the pin for YELLOW_LED
